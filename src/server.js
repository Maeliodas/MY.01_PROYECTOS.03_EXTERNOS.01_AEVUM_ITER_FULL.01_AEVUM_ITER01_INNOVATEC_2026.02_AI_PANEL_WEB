...
const panelVersion = 'APPROD4.1.8_R20_IN';
...
function requireAdmin(req, res, next) {
  ...
  const token = parseCookies(req).aevum_iter_admin;
...
app.get('/health', async (_req, res) => {
  try {
    ...
    res.status(503).json({ ok: false, service: 'aevum-iter-panel' });
  }
});
...
async function dashboardData(query = {}) {
  ...
  const [careerOptions] = await pool.query(`SELECT id, name FROM careers WHERE active=1 ORDER BY name`, []);
  ...
  return { totals, careers, profiles, profileCodes: profileCodes.map(r => r.code), careerOptions, affinity, evaluations: recentEvaluations };
}
...
app.post('/login', (req, res) => {
  ...
  res.setHeader('Set-Cookie', `aevum_iter_admin=${encodeURIComponent(sessionToken())}; HttpOnly; SameSite=Lax; Path=/; Max-Age=28800`);
  res.redirect('/');
});
app.post('/logout', (_req, res) => {
  res.setHeader('Set-Cookie', 'aevum_iter_admin=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0');
  res.redirect('/login');
});
...
app.get('/api/admin/report.pdf', requireAdmin, async (req, res) => {
  try {
    ...
    const doc = new PDFDocument({
      size: 'A4',
      bufferPages: true,
      margins: { top: 118, bottom: 64, left: 50, right: 50 },
      info: {
        Title: 'Reporte de orientación vocacional — Aevum Iter',
        Author: 'Aevum Iter',
        Subject: 'Estadísticas de evaluaciones RIASEC filtradas',
        Creator: 'Aevum Iter Admin Panel',
      },
    });
    // Noto Sans autohospedada para el reporte.
    ...
    const filename = `reporte-aevum-iter-${today.getFullYear()}-${pad(today.getMonth() + 1)}-${pad(today.getDate())}.pdf`;
    ...
    // Paleta Aevum Iter (azules del logo brújula).
    const BLUE = '#0262FC';
    const NAVY = '#012A62';
    const GREEN = '#0262FC';   // serie de datos: azul principal
    const INK = '#1a2432';
    const MUTED = '#5a6675';
    const LINE = '#c9d3e0';
    const BAR = '#0262FC';
    ...
    const reportFolio = `AI-${today.getFullYear()}${pad(today.getMonth() + 1)}${pad(today.getDate())}`;

    // Encabezado genérico Aevum Iter: logotipo a la izquierda, folio y
    // fecha a la derecha. Sin membretes institucionales.
    const LOGO_ASPECT = 1;   // aevum_logo.png es cuadrado (círculo sobre fondo)
    function drawHeader() {
      const RX = pageW - marginR;
      const logoS = 52;
      doc.image(ASSET('aevum_logo.png'), marginL, 42, { width: logoS, height: logoS });
      doc.fillColor(NAVY).font('Noto-Bold').fontSize(17)
        .text('AEVUM ITER', marginL + logoS + 12, 48);
      doc.fillColor(MUTED).font('Noto').fontSize(9)
        .text('Reporte de orientación vocacional', marginL + logoS + 12, 70);
      doc.fillColor(INK).font('Noto').fontSize(9.5)
        .text(`Folio ${reportFolio}`, RX - 220, 50, { width: 220, align: 'right' });
      doc.fillColor(MUTED).font('Noto').fontSize(9)
        .text(issuedDate, RX - 220, 64, { width: 220, align: 'right' });
    ...
    // Pie genérico: filete azul + marca y paginación. Se anula el margen
    // inferior durante el dibujo para no crear páginas fantasma (ver nota
    // original sobre maxY() de PDFKit con bufferPages).
    function drawFooter() {
      ...
      for (let i = 0; i < range.count; i++) {
        ...
        doc.strokeColor(LINE).lineWidth(1)
          .moveTo(marginL, pageH - 52)
          .lineTo(RX, pageH - 52)
          .stroke();
        doc.fillColor(MUTED).font('Noto').fontSize(7.5);
        doc.text('Aevum Iter · Descubre tu camino', marginL, pageH - 44, { width: 300, align: 'left' });
        doc.text(`Página ${i + 1} de ${range.count}`, RX - 200, pageH - 44, { width: 200, align: 'right' });
        doc.fillColor(INK);
      }
    ...
    const scopeText = scopeParts.length
      ? `El presente documento consolida los resultados de las evaluaciones vocacionales (modelo RIASEC / Holland) registradas en el sistema Aevum Iter, limitados a los siguientes criterios de filtrado: ${scopeParts.join('; ')}.`
      : 'El presente documento consolida los resultados de las evaluaciones vocacionales (modelo RIASEC / Holland) registradas en el sistema Aevum Iter, sin criterios de filtrado adicionales: se incluyen todos los registros disponibles en la base de datos al momento de la generación.';
...
io.use((socket, next) => {
  ...
  const token = cookies.aevum_iter_admin;
