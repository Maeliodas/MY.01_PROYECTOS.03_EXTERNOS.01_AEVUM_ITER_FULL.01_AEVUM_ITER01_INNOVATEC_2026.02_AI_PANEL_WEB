...
if (careerForm) {
  ...
  const refreshCareerProfile = () => {
    ...
    if(canvas && typeof Chart!=='undefined') {
      if(!radar) radar=new Chart(canvas,{type:'radar',data:{labels:dims,datasets:[{label:'Perfil RIASEC',data:dims.map(d=>values[d]),borderColor:'#0262fc',backgroundColor:'rgba(2,98,252,.12)',pointBackgroundColor:'#0262fc'}]},options:{responsive:true,maintainAspectRatio:false,scales:{r:{beginAtZero:true,max:10,ticks:{stepSize:2}}},plugins:{legend:{display:false}}}});
...
function renderChartsFromData(payload) {
  ...
  const bar = (id, rows, horizontal = true) => {
    ...
    chartInstances[id] = new Chart(el, {
      type: 'bar',
      data: { labels: (rows || []).map(x => x.label), datasets: [{ data: (rows || []).map(x => x.value), backgroundColor: '#0262fc', borderRadius: 5, borderSkipped: false }] },
  ...
  const line = (id, rows) => {
    ...
    chartInstances[id] = new Chart(el, {
      type: 'line',
      data: { labels: (rows || []).map(x => x.label), datasets: [{ data: (rows || []).map(x => x.value), borderColor: '#0262fc', backgroundColor: 'rgba(2,98,252,.10)', fill: true, tension: .3, pointRadius: 4 }] },
...
(function initRealtime() {
  ...
  function showLiveToast(title, detail) {
    ...
    if (!toast) {
      ...
      toast.id = 'liveToast';
      toast.style.cssText = 'position:fixed;right:20px;bottom:20px;z-index:9999;background:#0A1F44;color:#fff;padding:12px 16px;border-radius:10px;font-size:13px;box-shadow:0 8px 24px rgba(0,0,0,.18);max-width:320px;opacity:0;transition:opacity .25s';
